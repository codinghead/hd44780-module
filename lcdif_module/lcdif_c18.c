...
*******************************************************************************/
                                        /* This functions is implemented in   */
                                        /* assembly code and hence is         */
                                        /* defined as extern                  */
                                        /* See pbif_c18.asm                   */
extern unsigned char    pbifGetBusMutex(char * pbIfFlag);
                                        /* This functions is implemented in   */
                                        /* assembly code and hence is         */
                                        /* defined as extern                  */
                                        /* See pbif_c18.asm                   */
extern void             pbifReturnBusMutex(char * pbIfFlag);
...
*
* Summary: 
*   Deinitialises the LCDIFC18 module after use
*
* See also:
*   lcdifDeinit()
*
...
LCDIFNUM lcdifCreate(LCDIFOBJ * const lcdIfObj)
{
    ...
    if(lcdIfObj != (LCDIFOBJ *) 0)
    {
        ...
        if (lcdIfObj->E_LAT == (volatile unsigned char *) 0)
        {
        ...
        else
        {
            goto cannot_create_if;
                                        ...
                                        /* Do we have a RW_LAT?              */
        if (lcdIfObj->pbIfObject->RW_LAT == (volatile unsigned char *) 0)
        {
            goto cannot_create_if;
        }
                                        /* Do we have a RS_LAT?              */
        if (lcdIfObj->pbIfObject->RS_LAT == (volatile unsigned char *) 0)
        {
            goto cannot_create_if;
        }
                                        /* Do we have a DATA_PORT?            */
        if (lcdIfObj->pbIfObject->DATA_PORT == (volatile unsigned char *) 0)
        {
            goto cannot_create_if;
        }
                                        /* Do we have a DATA_LAT?             */
        if (lcdIfObj->pbIfObject->DATA_LAT == (volatile unsigned char *) 0)
        {
            goto cannot_create_if;
        }
                                        /* Do we have a DATA_TRIS?            */
        if (lcdIfObj->pbIfObject->DATA_TRIS == (volatile unsigned char *) 0)
        {
            goto cannot_create_if;
        }
